...
public:
  ...
  std::vector<double> std_moments(AbscisseType *bins, uint order,
                                  bool no_clip = false);
  ...
  uint64_t get_alloc_memory_size() {
    return histogram.get_alloc_memory_size();
  };
...
protected:
  ...
  void float_to_hist(FloatType data, BinType *histogram, FloatType max,
                     FloatType bin_width);
};
