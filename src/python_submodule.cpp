...
#define PY_HISTOGRAM_FLOAT(BinType, FloatType)                                 \
  py::class_<Histogram<BinType, FloatType>>(m, "Histogram_" #BinType           \
                                               "_" #FloatType)                 \
      .def(py::init<uint, int, double>(), "nofbins"_a.noconvert() = 0,         \
           "n_threads"_a.noconvert(), "max"_a = 1.0)                           \
      .def("accumulate",                                                       \
           &Histogram<BinType, FloatType>::accumulate_py<FloatType>,           \
           "data"_a.noconvert())                                               \
      .def("reset", &Histogram<BinType, FloatType>::reset)                     \
      .def("get", &Histogram<BinType, FloatType>::share_py)                    \
      .def_static("abscisse", &Histogram<BinType, FloatType>::abscisse_py,     \
                  "max"_a.noconvert(), "nofbins"_a.noconvert())                \
      .def("std_moments",                                                      \
           (std::vector<double>(Histogram<BinType, FloatType>::*)(             \
               py::array_t<FloatType>, uint, bool)) &                          \
...
#define PY_HISTOGRAM2D_FLOAT(BinType, FloatType)                               \
  py::class_<Histogram2D<BinType, FloatType>>(m, "Histogram2D_" #BinType       \
                                                 "_" #FloatType)               \
      .def(py::init<uint, int, double>(), "nofbins"_a.noconvert(),             \
           "n_threads"_a.noconvert(), "max"_a)                                 \
      .def("accumulate",                                                       \
           &Histogram2D<BinType, FloatType>::accumulate_py<FloatType>,         \
           "data_1"_a.noconvert(), "data_2"_a.noconvert())                     \
      ...
      .def("get_alloc_memory_size",                                            \
           &Histogram2D<BinType, FloatType>::get_alloc_memory_size);

#define PY_HISTOGRAM2D_UINT(BinType, IntegerType)                              \
  py::class_<Histogram2D<BinType, IntegerType>>(m, "Histogram2D_" #BinType     \
                                                   "_" #IntegerType)           \
      .def(py::init<int>(), "n_threads"_a.noconvert())                         \
      .def(py::init<int, uint>(), "n_threads"_a.noconvert(), "bit"_a = 0)      \
      .def("accumulate",                                                       \
           &Histogram2D<BinType, IntegerType>::accumulate_py<IntegerType>,     \
           "data_1"_a.noconvert(), "data_2"_a.noconvert())                     \
      ...
      .def("get_alloc_memory_size",                                            \
           &Histogram2D<BinType, IntegerType>::get_alloc_memory_size);
...
#define MOMENTS(HistType, AbscisseType)                                        \
  m.def("std_moments",                                                         \
        (std::vector<double>(*)(np_array<HistType> & histogram,                \
                                np_array<AbscisseType> &, uint, bool))(        \
            &std_moments_py<HistType, AbscisseType>),                          \
        "histogram"_a.noconvert(), "bins"_a.noconvert(),                       \
        "order"_a.noconvert(), "no_clip"_a.noconvert() = false);               \
  m.def(                                                                       \
      "moment",                                                                \
      (double (*)(np_array<HistType> & histogram, np_array<AbscisseType> &,    \
                  uint, uint64_t, bool))(&moment_py<HistType, AbscisseType>),  \
      "histogram"_a.noconvert(), "bins"_a.noconvert(), "exp"_a.noconvert(),    \
      "n_total"_a.noconvert(), "no_clip"_a.noconvert() = false);               \
  m.def("moment",                                                              \
        (double (*)(np_array<HistType> & histogram, np_array<AbscisseType> &,  \
                    uint, uint, uint64_t, int,                                 \
                    bool))(&moment_py<HistType, AbscisseType>),                \
        "histogram"_a.noconvert(), "bins"_a.noconvert(),                       \
        "exp_x"_a.noconvert(), "exp_y"_a.noconvert(), "n_total"_a.noconvert(), \
        "n_threads"_a.noconvert(), "no_clip"_a.noconvert() = false);           \
