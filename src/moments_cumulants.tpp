...
std::vector<double> std_moments(BinType *histogram, AbscisseType *bins,
                                uint n_bins, uint order, bool no_clip) {
  /*
...
template <class BinType, class AbscisseType>
double moment(BinType *histogram, AbscisseType *bins, uint n_bins, uint exp,
...
template <class BinType, class AbscisseType>
double centered_moment(BinType *histogram, AbscisseType *bins, uint n_bins,
...
template <class BinType, class AbscisseType>
double moment(BinType *histogram, AbscisseType *bins, uint n_bins, uint exp_x,
...
double centered_moment(BinType *histogram, AbscisseType *bins, uint n_bins,
  ...
  uint last_bin = no_clip ? n_bins - 1 : n_bins;
  AbscisseType x_bar =
      moment(histogram, bins, n_bins, 1, 0, n_total, n_threads, no_clip);
  AbscisseType y_bar =
      moment(histogram, bins, n_bins, 0, 1, n_total, n_threads, no_clip);
#pragma omp parallel
  {
    manage_thread_affinity();
    double val_local = 0;
...
#pragma omp for
    for (uint j = first_bin; j < last_bin; j++) {
      ...
      for (uint i = first_bin; i < last_bin; i++) {
        val_local +=
            histogram[n_bins * j + i] * pow(bins[i] - x_bar, exp_x) * y_power;
      }
    }
#pragma omp atomic update
    val += val_local;
