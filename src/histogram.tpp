...
Histogram<BinType, DataType>::Histogram(uint nofbins, int n_threads,
                                        ConstructorType max)
    : nofbins(nofbins), n_threads(n_threads),
      histogram(Multi_array<BinType, 1, uint32_t>(nofbins)), max(max), bit(0) {
  reset();
}
...
Histogram<BinType, DataType>::Histogram(int n_threads)
    : nofbins(1 << (8 * sizeof(ConstructorType))), n_threads(n_threads),
      histogram(Multi_array<BinType, 1, uint32_t>(
...
Histogram<BinType, DataType>::Histogram(int n_threads, uint bit)
    : nofbins(1 << bit), n_threads(n_threads),
      histogram(Multi_array<BinType, 1, uint32_t>(1 << bit)), max(1.0),
      bit(bit) {
  reset();
}
...
#define ACCUMULATE_DOUBLE(BIN_TYPE)                                            \
  ...
  void Histogram<BIN_TYPE, double>::accumulate(double *data, uint64_t L_data,  \
    ...
    _Pragma("omp parallel") {                                                  \
      ...
      _Pragma(                                                                 \
          "omp for reduction(+:histogram_local[:nofbins])") for (uint64_t i =  \
                                                                     0;        \
                                                                 i < L_data;   \
                                                                 i++) {        \
        float_to_hist(*((double *)(((char *)data) + (stride * i))),            \
...
#define ACCUMULATE_FLOAT(BIN_TYPE, UNROLL)                                     \
  ...
  void Histogram<BIN_TYPE, float>::accumulate(float *data, uint64_t L_data,    \
    ...
    for (uint64_t i = L_data - (L_data % UNROLL); i < L_data; i++) {           \
      float_to_hist(*((float *)(((char *)data) + (stride * (i)))),             \
                    histogram.get_ptr(), max, bin_width);                      \
    }                                                                          \
  }
...
#define ACCUMULATE_UINT8(BIN_TYPE, UNROLL)                                     \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram<BIN_TYPE, uint8_t>::accumulate(                               \
      uint8_t *data, uint64_t L_data, size_t stride) {                         \
    BIN_TYPE *histogram_local = histogram.get_ptr();                           \
    _Pragma("omp parallel") {                                                  \
      manage_thread_affinity();                                                \
      _Pragma(                                                                 \
          "omp for reduction(+:histogram_local[:1<<8])") for (uint64_t i = 0;  \
                                                              i <              \
                                                              L_data -         \
                                                                  (L_data %    \
                                                                   UNROLL);    \
...
#define ACCUMULATE_UINT16(BIN_TYPE, UNROLL)                                    \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram<BIN_TYPE, uint16_t>::accumulate(                              \
      uint16_t *data, uint64_t L_data, size_t stride) {                        \
    int b = bit;                                                               \
    BIN_TYPE *histogram_local = histogram.get_ptr();                           \
    _Pragma("omp parallel") {                                                  \
      manage_thread_affinity();                                                \
      _Pragma(                                                                 \
          "omp for reduction(+:histogram_local[:1<<b])") for (uint64_t i = 0;  \
                                                              i <              \
                                                              L_data -         \
                                                                  (L_data %    \
                                                                   UNROLL);    \
...
// template<>
// template<>
// void Histogram<BIN_TYPE,uint16_t>::accumulate( uint16_t* data, uint64_t
// L_data , int tail )
// {
// int b = bit;
// int tail = 16-b;
  ...
  void Histogram<BIN_TYPE, int8_t>::accumulate(int8_t *data, uint64_t L_data,  \
    ...
    _Pragma("omp parallel") {                                                  \
      ...
      _Pragma(                                                                 \
        ...
        PRAGMA_GCC_UNROLL(UNROLL)                                              \
        for (uint64_t j = 0; j < UNROLL; j++) {                                \
          histogram_local[(int)(*((uint8_t *)(((char *)data) +                 \
                                              (stride * (i + j))))) +          \
                          min_val]++;                                          \
        }                                                                      \
      }                                                                        \
    ...
    for (uint64_t i = L_data - (L_data % UNROLL); i < L_data; i++) {           \
      histogram[(int)(*((uint8_t *)(((char *)data) + (stride * (i))))) +       \
                min_val]++;                                                    \
    }                                                                          \
  }
...
#define ACCUMULATE_INT16(BIN_TYPE, UNROLL)                                     \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram<BIN_TYPE, int16_t>::accumulate(                               \
      int16_t *data, uint64_t L_data, size_t stride) {                         \
    int b = bit;                                                               \
    int min_val = 1 << (b - 1);                                                \
    BIN_TYPE *histogram_local = histogram.get_ptr();                           \
    _Pragma("omp parallel") {                                                  \
      ...
      _Pragma(                                                                 \
        ...
        PRAGMA_GCC_UNROLL(UNROLL)                                              \
        for (uint64_t j = 0; j < UNROLL; j++) {                                \
          histogram_local[(int)(*((int16_t *)(((char *)data) +                 \
                                              (stride * (i + j))))) +          \
                          min_val]++;                                          \
        }                                                                      \
      }                                                                        \
    ...
    for (uint64_t i = L_data - (L_data % UNROLL); i < L_data; i++) {           \
      histogram[(int)(*((int16_t *)(((char *)data) + (stride * (i))))) +       \
                min_val]++;                                                    \
    }                                                                          \
  }
...
// template<>
// template<>
// void Histogram<BIN_TYPE, int16_t>::accumulate( int16_t* data,  uint64_t
// L_data , int tail )
// {
// int b = bit;
// int tail = 16-b ;
...
inline void Histogram<BinType, Datatype>::float_to_hist(FloatType data,
                                                        BinType *histogram,
                                                        FloatType max,
                                                        FloatType bin_width) {
  /*
...
std::vector<double>
Histogram<BinType, Datatype>::std_moments(AbscisseType *bins, uint order,
                                          bool no_clip) {
...
void Histogram<BinType, Datatype>::accumulate_py(
    py::array_t<AccumulateType> data) {
...
template <class AbscisseType>
std::vector<double>
Histogram<BinType, Datatype>::std_moments_py(py::array_t<AbscisseType> bins,
