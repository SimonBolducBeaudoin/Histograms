...
Histogram2D<BinType, DataType>::Histogram2D(uint nofbins, int n_threads, ConstructorType max,
                                            uint n_prod)
    : n_prod(n_prod), nofbins(nofbins), n_threads(n_threads),
      histogram(Multi_array<BinType, 3>(n_prod, nofbins, nofbins)),
      hs(Multi_array<uint8_t, 4>(n_prod, n_threads, nofbins, nofbins)), max(max), bit(0) {
...
template <class BinType, class DataType>
template <class ConstructorType, class Enable>
Histogram2D<BinType, DataType>::Histogram2D(int n_threads, uint n_prod)
    : n_prod(n_prod), nofbins(1 << (8 * sizeof(ConstructorType))), n_threads(n_threads),
      histogram(Multi_array<BinType, 3>(n_prod, 1 << (8 * sizeof(ConstructorType)),
                                        1 << (8 * sizeof(ConstructorType)))),
      hs(Multi_array<uint8_t, 4>(n_prod, n_threads, 1 << (8 * sizeof(ConstructorType)),
                                 1 << (8 * sizeof(ConstructorType)))),
      max(1.0), bit(8) {
    omp_set_num_threads(n_threads);
    reset();
...
template <class BinType, class DataType>
template <class ConstructorType, class Enable>
Histogram2D<BinType, DataType>::Histogram2D(int n_threads, uint bit, uint n_prod)
    : n_prod(n_prod), nofbins(1 << bit), n_threads(n_threads),
      histogram(Multi_array<BinType, 3>(n_prod, 1 << bit, 1 << bit)),
      hs(Multi_array<uint8_t, 4>(n_prod, n_threads, 1 << bit, 1 << bit)), max(1.0), bit(bit) {
    omp_set_num_threads(n_threads);
    reset();
    reset_threads();
}
...
inline void Histogram2D<BinType, DataType>::to_hs(FloatType data_y, FloatType data_x, FloatType max,
                                                  FloatType bin_width, uint i_prod,
                                                  int this_thread) {
    uint binx;
    uint biny;
...
template <class BinType, class DataType>
inline void Histogram2D<BinType, DataType>::to_middleman(uint i_prod, int this_thread, uint biny,
                                                         uint binx) {
...
template <class BinType, class DataType>
template <class FloatType>
inline void Histogram2D<BinType, DataType>::compute_bins(FloatType data_2, FloatType data_1,
...
#define ACCUMULATE_DOUBLE(BIN_TYPE, UNROLL)                                                        \
    template <>                                                                                    \
    template <>                                                                                    \
    void Histogram2D<BIN_TYPE, double>::accumulate(double *data_1, double *data_2,                 \
...
#define ACCUMULATE_UINT8(BIN_TYPE, UNROLL)                                                         \
    template <>                                                                                    \
    template <>                                                                                    \
    void Histogram2D<BIN_TYPE, uint8_t>::accumulate(uint8_t *data_1, uint8_t *data_2,              \
                                                    uint64_t L_data, uint i_prod) {                \
        BIN_TYPE *histogram_local = histogram(i_prod);                                             \
        _Pragma("omp parallel") {                                                                  \
            manage_thread_affinity();                                                              \
...
#define ACCUMULATE_UINT16(BIN_TYPE, UNROLL)                                                        \
    template <>                                                                                    \
    template <>                                                                                    \
    void Histogram2D<BIN_TYPE, uint16_t>::accumulate(uint16_t *data_1, uint16_t *data_2,           \
                                                     uint64_t L_data, uint i_prod) {               \
        _Pragma("omp parallel") {                                                                  \
            manage_thread_affinity();                                                              \
            int this_thread = omp_get_thread_num();                                                \
...
#define ACCUMULATE_INT8(BIN_TYPE, UNROLL)                                                          \
    template <>                                                                                    \
    template <>                                                                                    \
    void Histogram2D<BIN_TYPE, int8_t>::accumulate(int8_t *data_1, int8_t *data_2,                 \
                                                   uint64_t L_data, uint i_prod) {                 \
        int min_val = 1 << (8 - 1);                                                                \
        BIN_TYPE *histogram_local = histogram(i_prod);                                             \
        _Pragma("omp parallel") {                                                                  \
...
#define ACCUMULATE_INT16(BIN_TYPE, UNROLL)                                                         \
    template <>                                                                                    \
    template <>                                                                                    \
    void Histogram2D<BIN_TYPE, int16_t>::accumulate(int16_t *data_1, int16_t *data_2,              \
                                                    uint64_t L_data, uint i_prod) {                \
        int min_val = 1 << (bit - 1);                                                              \
        _Pragma("omp parallel") {                                                                  \
            manage_thread_affinity();                                                              \
...
template <class BinType, class DataType> void Histogram2D<BinType, DataType>::reset() {
    for (uint k = 0; k < n_prod; k++) {
        for (uint j = 0; j < nofbins; j++) {
            for (uint i = 0; i < nofbins; i++) {
                histogram(k, j, i) = 0;
...
template <class BinType, class DataType> void Histogram2D<BinType, DataType>::reset_threads() {
...
template <class BinType, class Datatype> uint64_t Histogram2D<BinType, Datatype>::how_much_clip() {
    uint64_t clip = 0;
...
void Histogram2D<BinType, DataType>::accumulate_py(py::array_t<AccumulateType> data_1,
    ...
    accumulate((AccumulateType *)buf_1.ptr, (AccumulateType *)buf_2.ptr, L_data, i_prod);
}
