...
Histogram2D<BinType, DataType>::Histogram2D(uint nofbins, int n_threads,
                                            ConstructorType max)
    : nofbins(nofbins), n_threads(n_threads),
      histogram(Multi_array<BinType, 2>(nofbins, nofbins)),
      hs(Multi_array<uint8_t, 3>(n_threads, nofbins, nofbins)), max(max),
      bit(0) {
...
Histogram2D<BinType, DataType>::Histogram2D(int n_threads)
    : nofbins(1 << (8 * sizeof(ConstructorType))), n_threads(n_threads),
      histogram(Multi_array<BinType, 2>(1 << (8 * sizeof(ConstructorType)),
                                        1 << (8 * sizeof(ConstructorType)))),
      hs(Multi_array<uint8_t, 3>(n_threads, 1 << (8 * sizeof(ConstructorType)),
                                 1 << (8 * sizeof(ConstructorType)))),
      max(1.0), bit(8) {
  omp_set_num_threads(n_threads);
  reset();
...
Histogram2D<BinType, DataType>::Histogram2D(int n_threads, uint bit)
    : nofbins(1 << bit), n_threads(n_threads),
      ...
      hs(Multi_array<uint8_t, 3>(n_threads, 1 << bit, 1 << bit)), max(1.0),
      bit(bit) {
  omp_set_num_threads(n_threads);
  reset();
  reset_threads();
...
inline void
Histogram2D<BinType, DataType>::to_hs(FloatType data_y, FloatType data_x,
...
template <class BinType, class DataType>
inline void Histogram2D<BinType, DataType>::to_middleman(int this_thread,
                                                         uint biny, uint binx) {
  if (hs(this_thread, biny, binx) == 255) {
    _Pragma("omp atomic update") histogram(biny, binx) += (1 << 8);
  }
...
template <class FloatType>
inline void
Histogram2D<BinType, DataType>::compute_bins(FloatType data_2, FloatType data_1,
                                             FloatType max, FloatType bin_width,
                                             uint &biny, uint &binx) {
  /*
  ...
  */
  binx = std::abs(data_1) >= max ? 0
                                 : (unsigned int)((data_1 + max) / (bin_width));
  biny = std::abs(data_2) >= max ? 0
                                 : (unsigned int)((data_2 + max) / (bin_width));
}
...
#define ACCUMULATE_DOUBLE(BIN_TYPE, UNROLL)                                    \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram2D<BIN_TYPE, double>::accumulate(                              \
      double *data_1, double *data_2, uint64_t L_data) {                       \
    double bin_width = 2.0 * max / (nofbins);                                  \
    _Pragma("omp parallel") {                                                  \
      manage_thread_affinity();                                                \
      int this_thread = omp_get_thread_num();                                  \
      _Pragma("omp for") for (uint64_t i = 0; i < L_data - (L_data % UNROLL);  \
                              i += UNROLL) {                                   \
...
#define ACCUMULATE_FLOAT(BIN_TYPE, UNROLL)                                     \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram2D<BIN_TYPE, float>::accumulate(float *data_1, float *data_2,  \
                                                uint64_t L_data) {             \
...
#define ACCUMULATE_UINT8(BIN_TYPE, UNROLL)                                     \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram2D<BIN_TYPE, uint8_t>::accumulate(                             \
      uint8_t *data_1, uint8_t *data_2, uint64_t L_data) {                     \
    BIN_TYPE *histogram_local = histogram.get_ptr();                           \
    _Pragma("omp parallel") {                                                  \
      manage_thread_affinity();                                                \
      _Pragma(                                                                 \
          "omp for reduction(+:histogram_local[:1<<16])") for (uint64_t i = 0; \
                                                               i <             \
                                                               L_data -        \
                                                                   (L_data %   \
                                                                    UNROLL);   \
...
#define ACCUMULATE_UINT16(BIN_TYPE, UNROLL)                                    \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram2D<BIN_TYPE, uint16_t>::accumulate(                            \
      uint16_t *data_1, uint16_t *data_2, uint64_t L_data) {                   \
    _Pragma("omp parallel") {                                                  \
      manage_thread_affinity();                                                \
      int this_thread = omp_get_thread_num();                                  \
      _Pragma("omp for") for (uint64_t i = 0; i < L_data - (L_data % UNROLL);  \
...
#define ACCUMULATE_INT8(BIN_TYPE, UNROLL)                                      \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram2D<BIN_TYPE, int8_t>::accumulate(                              \
      int8_t *data_1, int8_t *data_2, uint64_t L_data) {                       \
    int min_val = 1 << (8 - 1);                                                \
    BIN_TYPE *histogram_local = histogram.get_ptr();                           \
    _Pragma("omp parallel") {                                                  \
      manage_thread_affinity();                                                \
...
#define ACCUMULATE_INT16(BIN_TYPE, UNROLL)                                     \
  template <>                                                                  \
  template <>                                                                  \
  void Histogram2D<BIN_TYPE, int16_t>::accumulate(                             \
      int16_t *data_1, int16_t *data_2, uint64_t L_data) {                     \
    int min_val = 1 << (bit - 1);                                              \
    _Pragma("omp parallel") {                                                  \
      manage_thread_affinity();                                                \
      int this_thread = omp_get_thread_num();                                  \
      _Pragma("omp for") for (uint64_t i = 0; i < L_data - (L_data % UNROLL);  \
        ...
        PRAGMA_GCC_UNROLL(UNROLL)                                              \
        for (uint64_t j = 0; j < UNROLL; j++) {                                \
          to_middleman(this_thread, (int)(data_2[i + j]) + min_val,            \
...
void Histogram2D<BinType, DataType>::accumulate_py(
    py::array_t<AccumulateType> data_1, py::array_t<AccumulateType> data_2) {
