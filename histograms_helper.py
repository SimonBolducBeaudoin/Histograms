...
import numba as _nb
@_nb.guvectorize([(_nb.uint32[:,:],_nb.float64[:],_nb.int32[:],_nb.float64,_nb.float64[:,:])], '(n,n),(n),(m),()->(m,m)')
def std_2Dmoments_numba(histogram,x,order,Cx,res=None):
    no_clip=True
    ...
    moments[0,1] = (hist * bins[None,x_slice]).sum()/ moments[0,0]
    moments[1,0] = (hist * bins[y_slice,None]).sum()/ moments[0,0]
    binx = (bins-moments[0,1])[None,x_slice] # centered bins
    biny = (bins-moments[0,1])[y_slice,None] # centered bins
    ...
    x_bar = moments[0,1]
    y_bar = moments[1,0]
    for j,oy in enumerate(order[3:]):
        for i,ox in enumerate(order[3:]):
            moments[j+3,i+3] = (hist* biny**oy * binx**ox).sum()  # <(x - <x>)^o>
        
    # normalisation
    moments[3:,3:] /= moments[0,0]
    
    stdize = _np.zeros( (len(order),len(order)), dtype=_np.float64)
    stdize = moments[2,0] ** (order[:,None] / 2.0) * moments[0,2] ** (order[None,:] / 2.0)
    # Standardisation
    moments[3:,3:] /= stdize[3:,3:]
    res[:] = moments
